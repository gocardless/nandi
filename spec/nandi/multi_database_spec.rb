...
RSpec.describe "Multi-database support" do
  ...
  describe "Configuration" do
    context "with multi-database configuration" do
      ...
      it "reports as multi-database enabled" do
        expect(Nandi.config.multi_database?).to be true
      end

      it "returns correct database names" do
        expect(Nandi.config.database_names).to eq([:primary, :analytics])
      end

      it "returns database-specific migration directories" do
        expect(Nandi.config.migration_directory_for(:primary)).to eq("db/safe_migrations/primary")
        expect(Nandi.config.migration_directory_for(:analytics)).to eq("db/safe_migrations/analytics")
        expect(Nandi.config.migration_directory_for(:nonexistent)).to eq("db/safe_migrations")
      end

      it "returns database-specific output directories" do
        expect(Nandi.config.output_directory_for(:primary)).to eq("db/migrate")
        expect(Nandi.config.output_directory_for(:analytics)).to eq("db/migrate/analytics")
      end
    end

    context "with single database configuration" do
      it "reports as single database" do
        expect(Nandi.config.multi_database?).to be false
      end

      it "returns default directories" do
        expect(Nandi.config.migration_directory_for(:any_db)).to eq("db/safe_migrations")
        expect(Nandi.config.output_directory_for(:any_db)).to eq("db/migrate")
      end
    end

    describe "validation" do
      it "validates database configurations" do
      ...
      it "requires migration_directory for each database" do
        config = Nandi::Config.new
        config.databases = {
  ...
  describe "Lockfile" do
    ...
    describe ".add" do
      it "stores database-specific entries" do
      ...
      it "stores non-database entries without prefix" do
        expect(Nandi::Lockfile.lockfile).to receive(:[]=).with(
  ...
  describe "Backward compatibility" do
    ...
    it "works without database declaration" do
      expect(migration.target_database).to be_nil
      expect { migration.up_instructions }.not_to raise_error
    end
    ...
    it "generates correct output path without database" do
      ...
      allow(compiled_migration).to receive(:migration).and_return(migration)
      allow(compiled_migration).to receive(:file_name).and_return("test.rb")
      
      expect(Nandi.config.output_directory_for(nil)).to eq("db/migrate")
    end
  end
end