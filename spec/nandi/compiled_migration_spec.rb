...
RSpec.describe Nandi::CompiledMigration do
  ...
  let(:lockfile) do
    lockfile_contents = {
      "20180104120000_my_migration.rb".to_sym => {
        source_digest: Digest::SHA256.hexdigest(source_contents),
        compiled_digest: Digest::SHA256.hexdigest(compiled_contents),
      }
    }
  ...
  describe "#compiled_digest" do
    ...
    it "returns the digest of the compiled migration" do
      ...
      expect(compiled_digest).to eq(Digest::SHA256.hexdigest(compiled_contents))
    end
  end
  ...
  describe "#source_digest" do
    ...
    it "returns the digest of the source migration" do
      ...
      expect(source_digest).to eq(Digest::SHA256.hexdigest(source_contents))
    end
  end
end